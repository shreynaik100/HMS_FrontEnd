...
export default function DoctorDashboard() {
	...
	const getAppointmentCount = async () => {
		const response = await axios.get(`${process.env.REACT_APP_SERVER_URL}/count/appointments`,
			{
				headers: {
	...
	const getPatientsTreatedCount = async () => {
		const response = await axios.get(`${process.env.REACT_APP_SERVER_URL}/count/patients/treated`,
			{
				headers: {
					authorization: `Bearer ${localStorage.getItem("token")}`
				}
	...
	const getPrescription = async () => {
        ...
        let response = await axios.post(`${process.env.REACT_APP_SERVER_URL}/prescriptions`,{},
            {
                headers: {
                    authorization: `Bearer ${localStorage.getItem("token")}`
                }
