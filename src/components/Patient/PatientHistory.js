...
function PatientHistory() {
  ...
  const getPatientById = async () => {
    ...
    const response = await axios.get(`${process.env.REACT_APP_SERVER_URL}/patients/${id}`);
  ...
  const getHistory = async () =>{
    let response = await axios.get(`${process.env.REACT_APP_SERVER_URL}/patients/history/${patientId}`, 
        {
            headers: {
